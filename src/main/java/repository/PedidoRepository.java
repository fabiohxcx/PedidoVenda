...
public class PedidoRepository implements Serializable {
	...
	public Map<String, BigDecimal> valoresTotaisPorVendedor() {
		Session session = this.manager.unwrap(Session.class);
		 ...
		 * .add(Projections.sqlGroupProjection("vendedor_id as vendedor",
		 * "vendedor", new String[] { "vendedor" }, new Type[] {
		...
		for (UsuarioValor usuarioValor : valoresPorUsuario) {
			resultado.put(usuarioValor.getUsuario(), usuarioValor.getValor());
		}
